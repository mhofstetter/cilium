...
func NewLocalIPIdentityWatcher(in struct {
	...
	if !watcher.client.IsEnabled() {
		return watcher
	}

	// Start watcher for endpoint IP --> identity mappings in key-value store.
	// Depending on the IPCache ensures that the ipcache map has been recreated before starting the watch.
	in.JobGroup.Add(job.OneShot("watch-local-identities", watcher.Watch, job.WithShutdown()))
