...
func (p *Proxy) ackProxyPort(ctx context.Context, name string, pp *ProxyPort) error {
	...
	if pp.rulesPort != pp.proxyPort {
		...
		if err := p.datapathUpdater.InstallProxyRules(ctx, pp.proxyPort, pp.ingress, true, name); err != nil {
...
func (p *Proxy) AllocateProxyPort(name string, ingress bool) (uint16, error) {
	...
	pp := p.proxyPorts[name]
	if pp == nil {
		pp = &ProxyPort{proxyType: types.ProxyTypeCRD, ingress: ingress}
	}
...
func (p *Proxy) ReinstallIPTablesRules(ctx context.Context) error {
	...
	for name, pp := range p.proxyPorts {
		if pp.rulesPort > 0 {
			// This should always succeed if we have managed to start-up properly
			if err := p.datapathUpdater.InstallProxyRules(ctx, pp.rulesPort, pp.ingress, true, name); err != nil {
				return fmt.Errorf("cannot install proxy rules for %s: %w", name, err)
			}
		}
	}
