...
type BPFMapsSizeConfig interface {
	GetDynamicSizeCalculator() func(def int, min int, max int) int
...
func (r *bpfMapsSizeConfig) calculateBPFMapSizes() error {
	...
	vms, err := memory.Get()
	if err != nil || vms == nil {
		logging.Fatal(r.logger, "Failed to get system memory", logfields.Error, err)
	}
	r.calculateDynamicBPFMapSizes(vms.Total, dynamicSizeRatio)

	return nil
}

func (r *bpfMapsSizeConfig) GetDynamicSizeCalculator() func(def int, min int, max int) int {
	vms, err := memory.Get()
	if err != nil || vms == nil {
		logging.Fatal(r.logger, "Failed to get system memory", logfields.Error, err)
	}

	return r.getDynamicSizeCalculator(r.flags.BPFMapDynamicSizeRatio, vms.Total)
}
...
func (r *bpfMapsSizeConfig) calculateDynamicBPFMapSizes(totalMemory uint64, dynamicSizeRatio float64) {
	getEntries := r.getDynamicSizeCalculator(dynamicSizeRatio, totalMemory)
	...
	if r.flags.BPFCTGlobalTCPMax == 0 {
		r.actualBPFCTGlobalTCPMax = getEntries(ctMapEntriesGlobalTCPDefault, limitTableAutoGlobalTCPMin, LimitTableMax)
	...
	if r.flags.BPFCTGlobalAnyMax == 0 {
		r.actualBPFCTGlobalAnyMax = getEntries(ctMapEntriesGlobalAnyDefault, limitTableAutoGlobalAnyMin, LimitTableMax)
	...
	if r.flags.BPFNATGlobalMax == 0 {
		r.actualBPFNATGlobalMax = getEntries(natMapEntriesGlobalDefault, limitTableAutoNatGlobalMin, LimitTableMax)
