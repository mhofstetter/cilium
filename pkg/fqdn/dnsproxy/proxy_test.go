...
func TestFullPathDependence(t *testing.T) {
	...
	model2 := newTestEndpointModel(int(epID3), endpoint.StateReady)
	ep3, err := endpoint.NewEndpointFromChangeModel(t.Context(), nil, nil, nil, nil, nil, nil, nil, identitymanager.NewIDManager(), nil, nil, s.repo, testipcache.NewMockIPCache(), &endpoint.FakeEndpointProxy{}, testidentity.NewMockIdentityAllocator(nil), ctmap.NewFakeGCRunner(), model2)
	require.NoError(t, err)

	ep3.Start(uint16(model.ID))
	t.Cleanup(ep3.Stop)
