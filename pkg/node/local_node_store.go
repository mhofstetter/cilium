...
type LocalNodeSynchronizer interface {
	...
	WaitForNodeIPAMAllocationCIDR(context.Context, *LocalNodeStore) error
}
...
func NewLocalNodeStore(params LocalNodeStoreParams) (*LocalNodeStore, error) {
	...
	s := &LocalNodeStore{db: params.DB, nodes: params.Nodes, sync: params.Sync}
...
func (s *LocalNodeStore) WaitForNodeIPAMAllocationCIDR(ctx context.Context) error {
	return s.sync.WaitForNodeIPAMAllocationCIDR(ctx, s)
}
...
func NewTestLocalNodeStore(mockNode LocalNode) *LocalNodeStore {
	...
	txn.Commit()
	return &LocalNodeStore{db: db, nodes: tbl, sync: &nopLocalNodeSynchronizer{}}
}
...
// WaitForNodeIPAMAllocationCIDR implements LocalNodeSynchronizer.
func (n nopLocalNodeSynchronizer) WaitForNodeIPAMAllocationCIDR(ctx context.Context, store *LocalNodeStore) error {
	return nil
}
