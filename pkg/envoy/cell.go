...
func registerSecretSyncer(params syncerParams) error {
	...
	for ns := range namespaces {
		secretResource, err := newK8sSecretResource(params.Lifecycle, params.K8sClientset, ns)
		if err != nil {
			return fmt.Errorf("failed to create k8s secret resource for namespace %s: %w", ns, err)
		}

		jobGroup.Add(job.Observer(
			fmt.Sprintf("k8s-secrets-%s-resource-events", ns),
...
func newK8sSecretResource(lc hive.Lifecycle, cs client.Clientset, namespace string) (resource.Resource[*slim_corev1.Secret], error) {
	if !cs.IsEnabled() {
		return nil, nil
	}
	...
	return resource.New[*slim_corev1.Secret](lc, lw, resource.WithMetric("Secret")), nil
}
