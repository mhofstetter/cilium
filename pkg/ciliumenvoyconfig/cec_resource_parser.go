	...
	for _, res := range xdsResources {
		...
		case envoy.ListenerTypeURL:
			...
			if !isInternalListener {
				...
				if !found {
					listener.ListenerFilters = append(listener.ListenerFilters, r.getListenerFilter(useOriginalSourceAddr, isL7LB))
				}
			}
...
func (r *cecResourceParser) getListenerFilter(useOriginalSourceAddr bool, l7lb bool) *envoy_config_listener.ListenerFilter {
